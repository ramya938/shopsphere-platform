...
def setup_logging():
    ...
    # Intercept logs from other loggers (e.g. uvicorn, sqlalchemy)
    logging.root.handlers = [InterceptHandler()]
    logging.root.setLevel(settings.LOG_LEVEL)

    # Disable other loggers' default handlers to avoid duplicate output
    for logger_name in ("uvicorn.error", "uvicorn.access", "sqlalchemy.engine"):
    ...
    if settings.ENVIRONMENT == "production":
        # Structured JSON logs for production/containers
        logger.add(
            sys.stdout,
        ...
        # Readable, colored logs for development
        logger.add(
            sys.stdout,
            level=settings.LOG_LEVEL,
